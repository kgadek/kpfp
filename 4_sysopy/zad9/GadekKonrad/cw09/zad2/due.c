...
int main(int argc, char* argv[]){
	...
	signal(SIGUSR1, myfirstsighandler);
	signal(SIGTERM, myfirstsighandler);
	signal(SIGKILL, myfirstsighandler);
	signal(SIGSTOP, myfirstsighandler);
...
void* searchForThatNeedle(void* args){
	...
	sigset_t mylocalsigset;
	thread = pthread_self();
	sigfillset(&mylocalsigset);
	pthread_sigmask(SIG_SETMASK, &mylocalsigset, NULL);
...
void myfirstsighandler(int sigid) {
	printf("AAAAAAAAAA dostałem w twarz sysgnałem %d!... Oczywiście panie władzo, już mówię: nazywam się PID:%d TID:%d\n",sigid, getpid(), (int)pthread_self());
}
