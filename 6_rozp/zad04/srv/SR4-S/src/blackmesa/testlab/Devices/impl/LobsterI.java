...
import blackmesa.testlab.devices._LobsterDisp;
...
public class LobsterI extends _LobsterDisp implements ITestLabServant {
	...
	public void move(String password, float direction, float speed,
		...
		System.out.format("Lobster:move(%.3f, %.3f)\n", direction, speed);
		note.processEvent("Lobster:move");
	}
	...
	public void growl(String password, int type, Current __current)
		...
		if(!passChk.check(password)) throw new WrongPasswordException();
		System.out.format("Lobster:growl(%d)\n", type);
		note.processEvent("Lobster:growl");
	}
