    ...
    for (i=0;i<n;i++)
	     {
	       ...
	       for(j=0;j<n;j++)
		 {
		   if(i!=j)mianownik[i]*=(tab[i][0]-tab[j][0]);	 
		}
	}	
  }
	...
	for(i=0;i<n;i++)
	  {
	    ...
	    for(j=0;j<n;j++)
	      {
		if(i!=j)element*=(tabout[k][0]-tab[j][0]);
	      }
	    tabout[k][1]+=element/mianownik[i]*tab[i][1];
	  }
      }
    return tabout;
  }
...
class intNewton
{
	...
	{
		if(i==k) return tab[k][1];
		else 
		{
			return (rekobliczIloraz(tab,i+1,k)-rekobliczIloraz(tab,i,k-1))/(tab[k][0]-tab[i][0]);
		}
	}
	void obliczIloraz(double **tab)
	{
		int i,j;
		/*ilorazRoznicowy[0]=rekobliczIloraz(tab,0,1);
		cout<<ilorazRoznicowy[0]<<endl;
		for(i=1;i<n-1;i++)
				{ilorazRoznicowy[i]=(rekobliczIloraz(tab,1,i+1)-ilorazRoznicowy[i-1])/(tab[i+1][0]-tab[0][0]);
				cout<<ilorazRoznicowy[i]<<endl;}*/
		for(i=0;i<n;i++)
			{
		...
		for(i=1;i<n;i++)
			for(j=1;j<i+1;j++)
			{
				ilorazRoznicowy[i][j]=(ilorazRoznicowy[i][j-1]-ilorazRoznicowy[i-1][j-1])/(tab[i][0]-tab[i-j][0]);
			}
	}
	double** obliczWyrazy(double** tab, int ile)
	{
		...
		for(k=0;k<ile;k++)
		{
			...
			{
				element*=(zm-tab[i][0]);
				tabout[k][1]+=element*ilorazRoznicowy[i+1][i+1];
			}
	}
	return tabout;
	}
...
int main(int argc, char* argv[])
{
  ...
  if(argv[3][0]=='L' && !argv[3][1])
  {
  	...
  	tabout=interp.obliczWyrazy(tab,ILE_PKT);
  }
  else
  {
	intNewton interp2(n);
	interp2.obliczIloraz(tab);
	tabout=interp2.obliczWyrazy(tab,ILE_PKT);
  }
  for(i=0;i<ILE_PKT;i++)
    {
