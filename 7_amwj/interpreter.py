...
def gc_h(mv, pos):
  if heap[pos] == Type.VART:
    nptr = allocate(4)
    mv[pos] = nptr
    heap[nptr:nptr+4] = heap[pos:pos+4]
    if not heap[pos+1] in mv: gc_h(mv, heap[pos+1])
    if not heap[pos+2] in mv: gc_h(mv, heap[pos+2])
    heap[nptr+1] = mv[heap[nptr+1]]
    heap[nptr+2] = mv[heap[nptr+2]]
  elif heap[pos] == Type.VARS:
  	nptr = allocate(2+heap[pos+1])
  	mv[pos] = nptr
  	heap[nptr:nptr+2+heap[pos+1]] = heap[pos:pos+2+heap[pos+1]]
...
def gc():
  ...
  global heapbegin
  gccnt = gccnt + 1
  with open('collect.txt', 'w') as f: f.write('{0}\n'.format(gccnt)) 
  mv = {0:0}
  
  if heaphead < heapend: heaphead = heaphalf
  else: heaphead = 1
  heapbegin = heaphead
  heapend = heaphead + heaphalf
  
  for var in variables:
    if not var in mv: gc_h(mv, variables[var])
    print "GC::: {0}: {1} -> {2}".format(var, variables[var], mv[variables[var]])
    variables[var] = mv[variables[var]]
...
class SVarTDecl(object):
  ...
  def run(self):
    try: ptr = allocate(4)
    except NotEnoughMemoryException:
      gc()
      ptr = allocate(4)
...
class SVarSDecl(object):
  ...
  def run(self):
    if self.value == 0:
      ...
      try: ptr = allocate(2+len(self.value))
      except NotEnoughMemoryException:
        gc()
...
class SHeapAnalyse(object):
  ...
  def run(self):
    global heapanalysecnt
    heapanalysecnt = heapanalysecnt + 1
    ptr = heapbegin
    typet = []
    types = []
    while ptr < heapend:
      if heap[ptr] == Type.VART:
        typet.append(heap[ptr+3])
        ptr = ptr + 4
      elif heap[ptr] == Type.VARS:
        slen = heap[ptr+1]
        types.append(''.join(heap[ptr+2:ptr+2+slen]))
        ptr = ptr + 2 + slen
      else:
        ptr = ptr + 1
    with open('heapAnalyze_{0}.txt'.format(str(heapanalysecnt)), 'w') as f:
      f.write('typeTVariables\n')
      f.write('\n'.join(map(lambda x: str(x), typet)))
      f.write('\ntypeSVariables\n')
      f.write('\n'.join(types))
      f.write('\n')
...
def p_statements(p):
  """statements : statement statements
                | statement
                |"""
  try: # jedno z tych moze sie wykrzaczyc
    p[0] = []
    p[0] = p[1:2]
    p[0].extend(p[2])
...
if __name__ == '__main__':
  heapsize = int(sys.argv[1].split('=')[1])
  heap     = [0 for i in range(heapsize)]
  heaphalf  = heapsize / 2
  heapend   = heaphalf + 1
