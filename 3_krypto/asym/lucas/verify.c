...
//	printf("\n"); 	for(i = 0 ; i < key_size ; ++i) printf("%c",binary_key[i]); 	printf("\n");
	...
	p = binary_key;
	d2i_DSAPublicKey(&dsa, &p, key_size);
  ...
  // wczytywanie podpisu pliku
	...
	printf("OK\n");
	int wynik = DSA_verify(0, hash, sizeof(hash), signature_bufor, signature_size, dsa);
	if(wynik == 1)
